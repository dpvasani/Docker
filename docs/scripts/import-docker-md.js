...
function writeDoc(destDir, index, srcFile, content) {
	...
	const slugBase = slugifyFileName(baseName);
	const destFile = path.join(destDir, `${seq}-${slugBase}.md`);

	const originalTitle = extractTitle(content) || slugBase.replace(/-/g, ' ');
	const numberedTitle = `${seq}. ${originalTitle}`;
	const body = stripFirstH1(content);

	const section = path.basename(destDir); // e.g., docker-cli
	const slug = `/${section}/${slugBase}`; // stable, unnumbered route
